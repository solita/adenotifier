...
def add_to_manifest(
    ...
    """
    # Put source object into Ade_Datasource class
    try:
        source_validated = adetypes.Ade_Datasource(**source)
    except Exception as e:
        logging.error("Error validating source object: {0}".format(e))
        raise
    # Set single_file_manifest based on configuration
    if (
        "single_file_manifest" in source["attributes"]
        and source["attributes"]["single_file_manifest"] is not None
    ):
    ...
    if not single_file_manifest:
        open_manifests = search_manifests(
            source_system_name=source_validated.attributes.ade_source_system,  # source["attributes"]["ade_source_system"],
            source_entity_name=source_validated.attributes.ade_source_entity,
            base_url=base_url,
    ...
    manifest = Manifest(
        base_url=base_url,
        source_system_name=source_validated.attributes.ade_source_system,
        source_entity_name=source_validated.attributes.ade_source_entity,
        format=source_validated.manifest_parameters.format,
        notify_api_key=notify_api_key,
        notify_api_key_secret=notify_api_key_secret,
    )

    # Set optional manifest attributes if configured in data source.
    # This could be also none if not configured.

    if (
        "columns" in source["manifest_parameters"]
        and source_validated.manifest_parameters.columns is not None
    ):
        manifest.columns = source["manifest_parameters"]["columns"]

    if (
        "compression" in source["manifest_parameters"]
        and source_validated.manifest_parameters.compression is not None
    ):
        manifest.compression = source["manifest_parameters"]["compression"]

    if (
        "delim" in source["manifest_parameters"]
        and source_validated.manifest_parameters.delim is not None
    ):
        manifest.delim = source["manifest_parameters"]["delim"]

    if (
        "fullscanned" in source["manifest_parameters"]
        and source_validated.manifest_parameters.fullscanned is not None
    ):
        manifest.fullscanned = source["manifest_parameters"]["fullscanned"]

    if (
        "skiph" in source["manifest_parameters"]
        and source_validated.manifest_parameters.skiph is not None
    ):
    ...
    if open_manifest_ids == []:
        ...
        if (
            "max_files_in_manifest" in source["attributes"]
            and source_validated.attributes.max_files_in_manifest is not None
        ):
            ...
            if (
                ...
                >= source_validated.attributes.max_files_in_manifest
            ):
    ...
    if (
        ...
        and source_validated.attributes.path_replace is not None
        and "path_replace_with" in source["attributes"]
        and source_validated.attributes.path_replace_with is not None
    ):
    ...
    if (
        "batch_from_file_path_regex" in source["attributes"]
        and source_validated.attributes.batch_from_file_path_regex is not None
    ):
...
def add_multiple_entries_to_manifest(
    ...
    if (
        "batch_from_file_path_regex" in source["attributes"]
        and source_validated.attributes.ade_source_system is not None
    ):
        # Parse entry specific batch number from file name if configured.
        try:
